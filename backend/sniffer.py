...
def get_ip_location(ip_address):
    if ip_address in ip_location_cache: return ip_location_cache[ip_address]
    ...
    try:
        ...
        loc = {"country": data.get('country', 'Unknown'), "city": data.get('city', 'Unknown'), "lat": data.get('lat', 0), "lon": data.get('lon', 0)}
        ip_location_cache[ip_address] = loc
        return loc
    except: return None
...
def packet_callback(packet):
    ...
    try:
        if IP in packet:
            ...
            dst_port = 0 # NEW: Initialize destination port

            protocol_name = "Other"
            if TCP in packet: 
                protocol_name = "TCP"; count_tcp += 1
                dst_port = packet[TCP].dport # Extract Port
            elif UDP in packet: 
                protocol_name = "UDP"; count_udp += 1
                dst_port = packet[UDP].dport # Extract Port
            elif ICMP in packet: protocol_name = "ICMP"; count_other += 1
            else: count_other += 1
            ...
            if packet.haslayer(DNS) and packet.haslayer(DNSQR):
                try: payload_content = f"[DNS] {packet[DNSQR].qname.decode('utf-8').rstrip('.')}"
                except: pass
            elif packet.haslayer(Raw):
                try: payload_content = packet[Raw].load.decode('utf-8', errors='ignore').strip()
                except: payload_content = "[Binary Data]"

            packet_data = {
                "timestamp": timestamp, "src": src_ip, "dst": dst_ip,
                "protocol": protocol_name, "payload": payload_content,
                "size": packet_size,
                "dst_port": dst_port # Pass port to detection engine
            }
...
def run_sniffer():
    ...
    while is_sniffing:
        try: sniff(prn=packet_callback, filter="ip", store=False, iface=IFACE, timeout=1)
        except:
            is_sniffing = False
            break
...
def start_sniffing():
    ...
    if not is_sniffing:
        captured_packets = []; total_packet_count = 0; count_tcp = 0; count_udp = 0; count_other = 0
